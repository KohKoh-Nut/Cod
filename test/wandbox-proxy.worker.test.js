...
describe("wandbox-proxy worker: POST / (compile and run)", () => {
    ...
    it("merges program_error and compiler_error into one field", async () => {
        mockFetchOk({ program_output: "", program_error: "boom", compiler_error: "warning: x" });
    ...
    it("rejects a request missing code", async () => {
        const req = makeRequest({ method: "POST", body: { compiler: "python-3.10" } });
        const res = await worker.fetch(req, makeEnv());
    ...
    it("rejects methods other than GET/POST/OPTIONS", async () => {
        const req = makeRequest({ method: "PUT", body: { code: "1+1", compiler: "python-3.10" } });
        const res = await worker.fetch(req, makeEnv());
...
describe("wandbox-proxy worker: GET routes", () => {
    ...
    it("filters /debug-compilers by language substring", async () => {
        ...
        const req = makeRequest({ method: "GET", path: "/debug-compilers?lang=python" });
        const res = await worker.fetch(req, makeEnv());
        const data = await res.json();

        expect(data).toEqual([{ name: "python-3.10", language: "Python", version: "3.10" }]);
    });
    ...
    it("returns an empty list from /debug-compilers when nothing matches", async () => {
        ...
        const req = makeRequest({ method: "GET", path: "/debug-compilers?lang=rust" });
        const res = await worker.fetch(req, makeEnv());
        const data = await res.json();
...
describe("wandbox-proxy worker: rate limiting", () => {
    ...
    it("allows requests under the limit and blocks the one that goes over", async () => {
        ...
        const send = () =>
            worker.fetch(
                makeRequest({ method: "POST", body: { code: "1+1", compiler: "python-3.10" } }),
                env,
            );
    ...
    it("tracks limits per IP, not globally", async () => {
        ...
        const sendFrom = (ip) =>
            worker.fetch(
                makeRequest({ method: "POST", ip, body: { code: "1+1", compiler: "python-3.10" } }),
                env,
            );
    ...
    it("fails open when no KV binding is configured", async () => {
        ...
        const req = makeRequest({ method: "POST", body: { code: "1+1", compiler: "python-3.10" } });
