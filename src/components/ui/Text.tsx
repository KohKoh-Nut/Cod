...
// layout-related classes (spacing, tracking, size) that vary by text variant
...
// color/background/border classes, kept separate from layout so they
...
// default html tag and styling per text type, so callers don't have to
// repeat "header should be bold and large" everywhere it's used
...
// used if type/level don't match any config above
...
// single component for all text styling in the app: headers, paragraphs,
// descriptions, dates, and links, all sharing the same variant system
export default function Text({
    ...
    // headers use a combined key like "header_1" to pick their config,
    // everything else uses the type directly
    const configKey =
    ...
    // explicit props win, otherwise fall back to the type's defaults
    ...
    // internal urls use Next's Link for client-side navigation
    ...
    // external links open in a new tab with the standard safety attributes
