...
import type { TerminalLine } from "@/hooks/editor/useCodeExecution";
...
// text color per terminal line type
...
// terminal-style panel showing run output, with an inline prompt when
// the running program is waiting on input
export default function CodeOutput({
    ...
    // keep the view pinned to the newest output line
    useEffect(() => {
    ...
    // move focus to the input as soon as a prompt appears
    useEffect(() => {
        if (waitingForInput) {
    ...
    // submits the typed line on Enter, like a real terminal
    ...
    return (
        <div className="font-mono text-xs w-full h-full flex flex-col overflow-hidden bg-bg text-fg rounded-none">
            <div className="font-bold text-xs uppercase tracking-wider w-full px-4 py-2 pt-3 flex items-center justify-between bg-bg-surface border-b border-border rounded-none">
                ...
                {/* only shown once loading has settled into an input prompt */}
            ...
            >
                {hasNoOutput ? (
                    ...
                    // each line is colored and prefixed based on its type
                ...
                {/* live input row, shown only while the program is blocked on input */}
                {waitingForInput && (
