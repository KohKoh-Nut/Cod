...
export default {
    async fetch(request, env) {
        ...
        if (await isRateLimited(ip, env)) {
            return cors(
                json({ error: "Too many requests, slow down." }, 429),
                origin,
            );
        }
        ...
        if (request.method === "GET" && url.pathname.endsWith("/runtimes")) {
            ...
            try {
                res = await fetch("https://wandbox.org/api/list.json", {
                    headers: {
                        "User-Agent":
                            "Mozilla/5.0 (compatible; WandboxProxy/1.0)",
                    },
                });
            } catch (err) {
                return cors(
                    json(
                        { error: `Could not reach Wandbox: ${err.message}` },
                        502,
                    ),
                    origin,
                );
        ...
        if (
            request.method === "GET" &&
            url.pathname.endsWith("/debug-compilers")
        ) {
        ...
        if (!code || !compiler) {
            return cors(
                json({ error: "Missing code or compiler" }, 400),
                origin,
            );
        }
        let wandboxRes;
        try {
            ...
            return cors(
                json({ error: `Could not reach Wandbox: ${err.message}` }, 502),
                origin,
            );
        }
        if (!wandboxRes.ok) {
            ...
            return cors(
                json(
                    { error: `Wandbox returned ${wandboxRes.status}: ${text}` },
                    502,
                ),
                origin,
            );
